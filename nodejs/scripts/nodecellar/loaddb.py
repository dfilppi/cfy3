...
ctx.logger.info("loading database")
...
# dbhosts are the mongod nodes in the mongo blueprint

dbhosts=ctx.instance.runtime_properties['dbhosts']
if(len(dbhosts)<1):
  raise NonRecoverableError('no mongods found')
  
ctx.logger.info("rtprops={}".format(str(ctx.instance.runtime_properties)))

mip=''
mport=int(0)
for k,v in ctx.instance.runtime_properties.iteritems():
  if( k.startswith('mongos_host_')):
    ctx.logger.info("connecting to mongos:{}".format(v))
    mip=v.split(':')[0]
    mport=int(v.split(':')[1])
    break
...
try:
  ...
  for dbhost in dbhosts.split(','):
    h,p,r=dbhost.split(':')
    if(not r in rsetdone):
      ctx.logger.info("adding shard {}/{}:{}".format(r,h,p))
      c.admin.command('addshard',"{}/{}:{}".format(r,h,p))
      rsetdone.add(r)
